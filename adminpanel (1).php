...
<div class = "login">
        ...
        <p style="text-align: center; padding-top: 20px; font-size: 15px;">Don't have account?<a href="adminsignup.php">Signup Here!</a></p>
</div>
<div class="sidebar"> 
        <div>
            <ul>
...
if(isset($_POST["submit"])) {    
    if(!empty($_POST['user']) && !empty($_POST['pass'])) {  
        ...
        $_con = mysqli_connect('localhost','root','root','admin') or die(mysqli_error());  

        // Select the database
        mysqli_select_db($_con, 'admin') or die("cannot select DB");  

        // Execute query
        $query = mysqli_query($_con, "SELECT * FROM adminlogin WHERE user='".$user."' AND pass='".$pass."'");  
